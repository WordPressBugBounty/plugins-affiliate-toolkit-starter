...
class BladeOne {    //<editor-fold desc="fields">
	...
	public function setBaseUrl( $baseUrl ) {
		$this->baseUrl = $baseUrl != null ?? rtrim( $baseUrl, '/' ); // base with the url trimmed
	...
	public function compile( $templateName = null, $forced = false ) {
		...
		if ( $forced || $this->isExpired( $templateName ) ) {
			...
			$contents = $this->compileString( $this->getFile( $template , $templateName) );
			if ( ! is_null( $this->compiledPath ) ) {
				...
				if ( ! $ok && $optimizedContent != '' ) {
					$this->showError( "Compiling", "Unable to save the file [{$compiled}]. Check the compile folder is defined and has the right permission  >>" . $contents . '<<' );
	...
	public function getTemplateFile( $templateName = '' ) {
		...
		$c            = count( $arr );
		if ( $c == 1 ) {
			return $this->findPath( $templateName . $this->fileExtension );
		} else {
			$file = $arr[ $c - 1 ];
			array_splice( $arr, $c - 1, $c - 1 ); // delete the last element
			$path = implode( '/', $arr );

			return $this->findPath( $path . '/' . $file . $this->fileExtension );
		}
	}

	private function findPath( $pathname ) {
		if ( is_array( $this->templatePath ) ) {
			foreach ( $this->templatePath as $temp ) {
				if ( file_exists( $temp . '/' . $pathname ) ) {
					return $temp . '/' . $pathname;
				}
			}

			return $this->templatePath[0] . '/' . $pathname;
		} else {
			return $this->templatePath . '/' . $pathname;
		}
	}
	...
	public function isExpired( $fileName ) {
		...
		if(is_numeric( $fileName )) {
			...
			$templatefound = get_post( $fileName );
			...
			if ( ! file_exists( $template )) {
				if(!ATKPSettings::$hideerrormessages)
				    $this->showError( "Read file", "Template not found: " . $this->fileName . ' in path: '.$fileName, true );
			}
		}
	 ...
	 * @return string
	 */
	public function getFile( $fileName ,$templateName= null) {
		...
		if($templateName != null && is_numeric( $templateName )) {
			...
			$templatepath = '';
			foreach ( $views as $view ) {
				if ( file_exists( $view . '/' . $templateName . '.blade.php' ) ) {
					$templatepath = $view . '/' . $templateName . '.blade.php';
					break;
				}
			}
			if($templatepath != '') {
	...
	protected function handleViewException( $e ) {
		...
		if ( atkp_options::$loader->get_hide_error_message() ) {
			...
			return "<strong>Fatal error:</strong>: " . $e->getMessage();
		}
	}
	//</editor-fold>
	...
	public static function e( $value ) {
		...
		return $value == null ? null : htmlentities( $value, ENT_QUOTES, 'UTF-8', false );
	}
	//</editor-fold>
	...
	public function showError( $id, $text, $critic = false ) {
		ob_get_clean();
		$error = "<div style='background-color: red; color: black; padding: 3px; border: solid 1px black;'>";
		$error = "BladeOne Error [{$id}]:<br>";
		$error = "<span style='color:white'>$text</span><br></div>\n";
		//if ( $critic ) {
		//	die( 1 );
		//}
		if ( ! atkp_options::$loader->get_hide_error_message() )
